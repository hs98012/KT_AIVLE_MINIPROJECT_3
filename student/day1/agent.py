...
def _extract_tickers_from_query(query: str) -> List[str]:
    ...
    # 전처리: 대문자 통일 + 특수문자 제거(공백 치환)
    cleaned = re.sub(r"[^A-Za-z0-9\s]", " ", query.upper())

    # 단일 패턴으로 영문(1~5자) 또는 숫자(6자리) 매칭
    pattern = re.compile(r"\b(?:[A-Z]{1,5}|\d{6})\b")

    seen = set()
    result: List[str] = []
    # finditer는 원문 내 매칭 '등장 순서'를 보장
...
def _normalize_kr_tickers(tickers: List[str]) -> List[str]:
    ...
    if isinstance(tickers, str):
        raise TypeError("tickers must be a list of strings, not a single string")
    if not isinstance(tickers, Iterable):
        raise TypeError("tickers must be an iterable of strings")
    ...
    for sym in tickers:
        ...
        s = sym.strip()
        out.append(f"{s}.KS" if re.fullmatch(r"\d{6}", s) else s)
    return out
...
def _handle(query: str) -> Dict[str, Any]:
    ...
    # 5) 실행 및 결과 반환(표준 스키마는 Day1Agent.handle → merge에서 보장)
...
def before_model_callback(
    ...
    **kwargs,
    ...
    try:
        ...
        err = f"Day1 에러: {e}"
        return LlmResponse(
            content=types.Content(parts=[types.Part(text=err)], role="model")
        )
...
day1_web_agent = Agent(
    name="WebAgent",
    model=MODEL,
    description="웹 검색, 주가 정보, 기업 개요 요약을 통합 제공하는 에이전트",
    instruction=(
        "입력된 사용자 질의에 대해 Tavily API를 활용한 웹 검색 결과와 "
        "yfinance 기반 주가 스냅샷, Tavily 기반 기업 개요 요약을 "
        "표준 마크다운 형식으로 작성하라.\n\n"
        "출력 지침:\n"
        "1) 결과는 **표준 마크다운**으로 구성한다.\n"
        "2) 주가·기업 정보는 표 또는 목록으로 간결히 정리한다.\n"
        "3) 가능한 경우 웹 검색 **출처 링크**를 명시한다.\n"
        "4) 어조는 간결하고 공식적으로 유지한다.\n"
        "5) 근거가 불충분한 항목은 '출처 미확인'으로 표시한다."
    ),
    tools=[],
